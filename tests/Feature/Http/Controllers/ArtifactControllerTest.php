...
// Index Tests
it('redirects guests to login when accessing artifacts index', function () {
...
// Show Tests
it('redirects guests to login when accessing artifact show', function () {
...
// Render Tests
it('redirects guests to login when accessing artifact render', function () {
    $artifact = Artifact::factory()->create();
...
it('renders unknown artifact type using code renderer as fallback', function () {
    ...
    $chat = Chat::factory()->for($user)->create();
    $message = Message::factory()->for($chat)->create();
    // Create artifact with unknown type directly in database
    $artifact = Artifact::factory()->for($message)->create(['type' => 'unknown_type']);

    $response = $this->actingAs($user)->get(route('artifacts.render', $artifact));

    $response->assertOk();
    // Default falls back to code renderer which uses the base CSP
    expect($response->headers->get('Content-Security-Policy'))->toContain("default-src 'self'");
});
